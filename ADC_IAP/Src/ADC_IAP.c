...
uint16_t readADC(ADC_ChannelTypedef channel) {
    if (channel != 0)//通道可以是零，方便调试故写判断，要用通道0条件改一下即可
    {
...
void checkPowerLoss(uint8_t cnt) {
    ...
    if (voltage < VOLTAGE_THRESHOLD) //如果电压低于4.0V
    {
...
AlarmSettings loadAlarmSettings(void) {
    ...
    setAlarm(settings.hour, settings.minute, settings.enabled);//导出数据内部处理
    return settings;//外部数据处理
}
