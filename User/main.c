...
void UpdateDisplay(uint8_t KeyValue)
{
    if (exKeyValue != 0XFF) // 松手前只出一次键
    {
        ...
        if (first_Press == 0)
        {
            if (KeyValue == 17)
            {
                GPIO_TogglePins(GPIOA, GPIO_Pin_12);  // 红外传感器开�?                GPIO_TogglePins(GPIOC, GPIO_Pin_10); // LED 灯翻�?
            ...
            else if (KeyValue == 18)
            {
                ...
                GPIO_SetBits(GPIOC, GPIO_Pin_4 | GPIO_Pin_5 | GPIO_Pin_10 | GPIO_Pin_11); // 先关闭所�?LED
                GPIO_SetBits(GPIOA, GPIO_Pin_7 | GPIO_Pin_8);                             // 先关闭所�?LED
                GPIO_SetBits(GPIOA, GPIO_Pin_9);//开马达
...
int main(void)
{
    ...
    while (1)
    {
        ...
        int fputc(int ch, FILE *f)
        {
            ...
            while (UART_GetFlagStatus(UART1, UART_Flag_TX) == RESET); // 等待发送完�?    UART_ClearFlag(UART1, UART_Flag_TX); // 清除发送完成标�?
            return ch;
        }
